...
boosting <- function(f0 = 0.5, lambda, n_tree, tree_depth, data) {
  ...
  # predição final
  f_hat <- unname(cbind(f0, cbind.data.frame(lapply(forest, predict)) * lambda))
...
predict.boosting <- function(boosting_result, k, type = c('classe', 'prob'),
  ...
  if (missing(k)) k <- length(boosting_result$forest) + 1
  type <- match.arg(type)
  if (k == 0) pred <- boosting_result$f0
  else {
    pred <- rowSums(  # tentar reescrever essa parte para aumentar eficiência, tentar usar vappaly, mudar cbind.data.frame para um comando mais eficiente
      cbind(boosting_result$f0, boosting_result$lambda *
              cbind.data.frame(lapply(boosting_result$forest[1:k],
                                      \(tree) predict(tree, newdata = data)))))
  }
...
plot.boosting <- function(boosting_result, col, newdata, add_plot = F) {
  ...
  for (i in 0:n_tree) {
    EQM[i+1] <- mean(
      (predict(boosting_result, k = i, type = 'prob', newdata = newdata) -
         newdata$y)^2)
  }
...
# ajustando modelos ---------------
boost1 <- boosting(f0 = 0.5, lambda = 0.5, n_tree = 5, tree_depth = 1, data = mnist)
boost2 <- boosting(f0 = 0.5, lambda = 0.5, n_tree = 15, tree_depth = 1, data = mnist)
boost3 <- boosting(f0 = 0.5, lambda = 0.5, n_tree = 30, tree_depth = 1, data = mnist)

# plot
plot(boost3)
plot(boost2, col = 'darkgray', add_plot = T)
plot(boost1, col = 'blue', add_plot = T)
```


```{r}
# debug(plot.boosting)
boost <- boosting(f0 = 0.5, lambda = 0.8, n_tree = 8, tree_depth = 1, data = mnist)
...
# ajustando boosting
n_tree <- 4
boost1 <- boosting(f0 = 0.5, lambda = 0.8, n_tree, tree_depth = 1, data = mnist)
boost2 <- boosting(f0 = 0.5, lambda = 0.2, n_tree, tree_depth = 1, data = mnist)

# computando EQM
pred1 <- vector(mode = 'numeric', length = n_tree + 1)
pred2 <- vector(mode = 'numeric', length = n_tree + 1)

for (i in 0:n_tree) {
  pred1[i + 1] <- mean((predict(boost1, k = i, type = 'prob') - mnist$y)^2)
  pred2[i + 1] <- mean((predict(boost2, k = i, type = 'prob') - mnist$y)^2)
}

# plot
plot(0:n_tree, pred1, ylab='EQM', type = 'b')
plot(0:n_tree, pred2, ylab='EQM', type = 'b')
```
