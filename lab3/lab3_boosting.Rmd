...
plot.boosting <- function(boosting_result, col, add_plot = F) {
...
```{r, echo=F, eval=F}
# testando função
boost <- boosting(0.5, lambda = 0.5, n_tree = 15, tree_depth = 1, data = mnist)
debug(plot.boosting)
plot(boost)
```
...
```{r Q6}
fit <- boosting(f0 = 0.5, lambda = 0.8, n_tree = 200, tree_depth = 1, data = mnist)
predicao <- predict(fit, type = 'classe')
table(predicao, mnist$y)  # matriz de confusão
plot(fit)
result <- data.frame(predicao = ifelse(predicao == 0, 3, 5))
write.csv(result, 'l3-pred-J.csv')
```
