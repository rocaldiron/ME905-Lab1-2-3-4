...
boosting <- function(f0 = 0.5, lambda, n_tree, tree_depth, data) {
  ...
  result <- list(data = data, EQM = EQM, lambda = lambda, forest = forest)
...
# k número de árvores a serem utilizadas
...
predict.boosting <- function(boosting_result, k, type = c('classe', 'prob'),
  ...
  type <- match.arg(type)
  if (type == 'classe') {
    lapply(boosting_result, \(tree) predict(tree, newdata = data))  # continuar aqui
  }
  else  # fazer caso para type = `prob`
}
debug(predict.boosting)
predict(boosting_result=boost, k=3, type='prob')
```
