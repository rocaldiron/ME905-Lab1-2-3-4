...
Pela matriz de confusão, erramos 49 produtos, o que gera uma acurácia de `r round(100 * (tab[1] + tab[4])/sum(tab), 2)`%.
...
valida <- read_csv('produtos_valida.csv')
pred <- apply(predict(fit, newdata = valida), MARGIN = 1, which.max) - 1
compra <- as.vector(pred < (val_venda - valida$cost)/val_venda)

despesa <- sum(valida$cost[compra])
venda_total <- sum(compra & !valida$y) * val_venda
