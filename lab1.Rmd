...
fsr <- function(data, p_max = 40) {
  ...
  EQM_pred <- numeric(p_max)            # salvar EQM minimo por iteração
  ...
  treino <- data[shuffle == 1,]  # 1 = treino
  teste <- data[shuffle == 2,]   # 2 = teste
  names_pred <- names(data[-1])  # nome de todas as preditoras
  
  for (p in 1:p_max) {
    names_pred <- setdiff(names_pred, pred_selecionado)  # preditores não selecionados
    ...
    best_fit <- lm(y ~ ., treino[c('y', pred_selecionado[1:p])])
    EQM_pred[p] <- sum((teste$y - predict(best_fit, newdata = teste))^2)/n_teste
    # EQM_pred[p] <- sum(lm(y ~ ., teste[c('y', pred_selecionado[1:p])])$residuals^2)/n_teste
  }
  
  result <- data.frame(p = 1:p_max, EQM = EQM_pred, pred = pred_selecionado)
...
```{r}
fsr_cv <- function(data, p_max = 5, k = 5) {
  # objetos
  pred_selecionado <- character(p_max)  # vetor com o nome dos pred selecionados
...
```{r, eval=F, echo=F}
# testes, reproduzindo oq eu foi feito no sldie
set.seed(2)
n <- 5000
