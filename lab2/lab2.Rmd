...
modelos <- list(
  ...
  modelo9 = random_forest(mnist, n_tree = 100, p = 50, maxdepth = 20)
)
...
Além disso, note que os modelos 7, 8 e 9 foram os melhores, justamente os que apresentam o maior número de árvores e preditoras por árvore. Além disso, o modelo 1 e 2 foram excepcionalmente ruins e são justamente os que apresentam o menor número de árvores e preditoras. Então, os parâmetros que mais parecem importar são: o número de árvores (`n_tree`) e quantidade de preditoras no treino (`p`). Ademais, também é possível perceber que aumentar o tamanho máximo da árvore nem sempre gera uma melhora, ocorrendo um acréscimo entre o modelo 1 para o 2; mas um descréscimo entre o modelo 3 para 4 e 5 para 6; e sem mudança significativa do modelo 7 para 8.
