...
a <- random_forest(mnist, n_tree=3)
```
...
```{r}
pred_tree <- vector(mode='list', length(a))

for (i in 1:length(a)) {
  pred_tree[[i]] <- unname(predict(a[[i]], type = 'class', newdata = mnist))
}

db_pred <- data.frame(y = mnist$y, tree = unname(do.call(cbind.data.frame,
                                                         pred_tree)))
```
